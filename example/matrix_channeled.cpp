...
int main()
{
    ...
    for (std::size_t i = 0; i < matrix.rows(); ++i) {
        for (std::size_t j = 0; j < matrix.columns(); ++j) {
            std::cout << "m[" << i << "][" << j << "] = (" << static_cast<int>(matrix(i, j)[0])
    ...
    std::cout << "width: " << converted_image.width() << " height: " << converted_image.height()
              << '\n';
    ...
    blaze::DynamicMatrix<blaze::StaticVector<double, 3>> mat(matrix);
    mat = mat % mat % mat % mat % mat;
    auto remapped = flash::remap_to_channeled<unsigned char>(mat);

    for (std::size_t i = 0; i < mat.rows(); ++i) {
        for (std::size_t j = 0; j < mat.columns(); ++j) {
            std::cout << "source[" << i << "][" << j << "] = (" << static_cast<int>(mat(i, j)[0])
                      << ' ' << static_cast<int>(mat(i, j)[1]) << ' '
                      << static_cast<int>(mat(i, j)[2]) << ") ";
        }
        std::cout << '\n';
    }

    for (std::size_t i = 0; i < remapped.rows(); ++i) {
        for (std::size_t j = 0; j < remapped.columns(); ++j) {
            std::cout << "remapped[" << i << "][" << j << "] = ("
                      << static_cast<int>(remapped(i, j)[0]) << ' '
                      << static_cast<int>(remapped(i, j)[1]) << ' '
                      << static_cast<int>(remapped(i, j)[2]) << ") ";
        }
        std::cout << '\n';
    }
}